...
- **Tres capas de control, válidas y verificadas:** (1) **sandbox/toolset**: Codex `-s read-only`; **Claude SIEMPRE con toolset cerrado** `--tools "Read,Grep,Glob"` (Bash fuera del toolset → read-only duro, **inmune a un `allow:["Bash"]` heredado** de otro scope, ronda 3 #7) → **Claude no señaliza por comando**; su fin lo detecta el conductor por `tui-idle`. La señal por comando queda **solo para Codex** (sandbox de kernel + `-a untrusted`); (2) **MCP**: Codex por **perfil instalado** `$CODEX_HOME/<nombre>.config.toml` invocado con **`-p <nombre>`** (no `-c`, que solo toma `key=value`) — claves **server-scoped** `mcp_servers.<id>.{enabled,enabled_tools,disabled_tools,default_tools_approval_mode}` + `mcp_servers.<id>.tools.<t>.approval_mode` (modos `auto|prompt|writes|approve`) **y** overrides por ID para `apps.*` y `plugins.*.mcp_servers.*`; Claude — **MCP por vigilancia manual (P4) en el default atendido** (S3/Fase 2: `--tools` acota solo built-ins, no gobierna MCP; el gate es el humano en la TUI, no un inventario/allowlist que mantener) + endurecimiento **opcional** `--strict-mcp-config --mcp-config <allowlist>` **solo para el caso desatendido** (verificado en vivo: allowlist vacío → `CERO-MCP`; da vuelta MCP a deny-all); nunca wildcards; (3) **hooks**: `disableAllHooks:true` (Claude) / `--disable hooks` (Codex). Toolset+hooks siempre; el gate de MCP es vigilancia manual (atendido) o strict-mcp-config (desatendido).
...
- [ ] `claude-readonly.settings.json`: `disableAllHooks:true` + **toolset cerrado** `--tools "Read,Grep,Glob"` (sin Bash → read-only duro **inmune a un `allow` heredado**, confirmado en ronda 4; Claude no señaliza, el fin lo detecta el conductor por `tui-idle`) **S3/Fase 2 (resuelto):** `claude-readonly.settings.json` queda mínimo (`{disableAllHooks:true}`); MCP se controla por **vigilancia manual** (P4) en el default atendido, con `--strict-mcp-config`+allowlist opcional para desatendido (ver Global Constraints, capa MCP). No se mantiene inventario de MCP (envejece). `claude-write.settings.json`: mismo worktree = `--permission-mode manual` + `Edit(./**)`/`Write(./**)`/`Bash(<proof>:*)`; `acceptEdits` **solo** worktree hermano aislado; su gate de MCP (vigilancia manual o strict-mcp-config desatendido) lo define Fase 5.
