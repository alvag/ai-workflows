...
- **Envelope con autoridad (round 12/Fase 2: envelope de texto = solo `nonce`; autoridad fuera de banda):** salida del modo + una línea `X-CMO: nonce=<..>` + `STATUS: done` como última línea no vacía. El conductor cosecha **solo** el mensaje que trae el `nonce` del dispatch en curso. El `nonce` es un **token de correlación** (es texto del modelo, falsificable); la **autoridad** la establece el conductor **fuera del texto**, antes de cosechar — `payload={taskId,dispatchId}`+`sender` del `worker_done` para Codex, propiedad de la sesión (`--session-id` propio) para Claude. El parser tolera `taskId`/`dispatchId` en el texto por compatibilidad, pero el dispatch `orca-session` solo pide `nonce` (el envelope de 3 campos de rondas previas queda **superseded**).
...
- [ ] `claude-readonly.settings.json`: `disableAllHooks:true` + **toolset cerrado** `--tools "Read,Grep,Glob"` (sin Bash → read-only duro **inmune a un `allow` heredado**, confirmado en ronda 4; Claude no señaliza, el fin lo detecta el conductor por `tui-idle`) + `ask`/`deny` por tool MCP de **escritura enumerada** (defensa en profundidad). (No se usa dump de permisos efectivos: `claude --print-permissions` **no existe** en 2.1.214, ronda 4 #6; el toolset cerrado **es** la garantía para built-ins.) **S3/Fase 2 (resuelto):** el toolset cerrado NO gobierna MCP → gate primario `claude-readonly.mcp.json` + `--strict-mcp-config` (ver Global Constraints, capa MCP). `claude-write.settings.json`: mismo worktree = `--permission-mode manual` + `Edit(./**)`/`Write(./**)`/`Bash(<proof>:*)`; `acceptEdits` **solo** worktree hermano aislado; su propio allowlist `--strict-mcp-config` lo define Fase 5.
...
**Produces:**
- `createOwnedSession({family,role,mode,worktree})` (ronda 3 #2) → crea la terminal fresca (`terminal create` con el perfil de 1.4) y registra `{terminalHandle,family,sessionId,transcriptPath,createdAt,uid}` en un **registro de sesión propia** del conductor. **Locator por familia (corregido en Fase 1 con evidencia live):** Claude resuelve el locator aquí, directo, por `--session-id` fijado. **Codex NO puede resolverlo aquí**: su rollout no existe hasta el primer turno (post-dispatch) — verificado en vivo (ver `skills/cross-model-orca/spikes/RESULTS.md` → Task 0.1 «TIMING del rollout») —, así que se registra con `createdAt` + `transcriptPath` pendiente y el rollout se resuelve **lazy** en `awaitDone` (creación+`cwd`+`mtime`, con retry acotado). 1 candidato → resuelve; 0 tras retry o >1 → `transcriptPath` null → la skill degrada a `cli`.
...
- [ ] Confirmar que una tool MCP de **escritura** (`deny`/`ask`) no corre sin gate; y que el **preflight** bloquea MCP/app/plugin no inventariado. **Gate primario Claude (S3):** confirmar en la sesión interactiva real que `--strict-mcp-config` deja **solo** los servidores del allowlist (env MCP invisible; `CERO-MCP` con allowlist vacío ya verificado headless) y que `context7` del allowlist **sí aparece** en la sesión interactiva (el probe `-p` no lo zanja por cold-start). **Commit.**
...
> **Trazabilidad de la review cross-model — APPROVED (ronda 6):** el plan pasó **6 rondas** de crítica adversarial de la otra familia (Codex por `codex exec` read-only, verificando contra Codex 0.144.6 / Claude 2.1.214), aplicadas con criterio del conductor. Trayectoria de hallazgos: **14 → 20 → 9 → 8 → 2 → 0 (APPROVED)**, con la severidad descendiendo de arquitectura (cosecha/hooks/ARG_MAX) a wiring y precisión de CLI. Ningún hallazgo fue refutado. Los detalles finos de implementación (algoritmo exacto de reconstrucción idempotente, formato del rollout) quedan como criterios con test dentro de sus tasks y se **fijan con los contratos del spike de Fase 0** (sin ellos, la rama `orca-session` degrada a CLI).
