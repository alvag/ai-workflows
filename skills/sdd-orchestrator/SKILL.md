...
- **Las cuatro opciones del checkpoint, en los dos gates de Fase 1.** Si la revisión devuelve
  ...
  **cerrar la revisión**. Las cuatro se ofrecen siempre: `disponibles: false` advierte que conceder
  no puede converger, no deshabilita nada. **El paso previo no las cambia.** **Si
  `aplicaciones_pendientes` es mayor que cero,
  ...
  arbitrar, ese conteo se re-deriva del ledger** —junto con sus IDs y los tres inventarios— porque el
  valor del retorno es anterior al arbitraje; `causa_corte` y `disponibles` **no** se re-derivan, son
  históricos, y `disponibles` deja de usarse como advertencia una vez que hubo arbitraje. Con el modo automático activo, el fin de tanda es una
