...
## Transporte de las corridas delegadas (propagar sin heredar)

El orquestador tiene acá dos obligaciones **contrarias** y las cumple las dos: **propaga** la intención
de transporte a los `sdd-flow` que delega, y **no** convierte a panes su propio fan-out por repos.

- **Se persiste** en el nivel del orquestador: clave top-level `cross_model` del `manifest.yml`
  (hermana de `cross_review` y `co_explore`), lo único que sobrevive a una sesión nueva.
- **Llega a cada `sdd-flow` delegado**: el valor resuelto por repo viaja en el prompt del fan-out
  (`reference.md` → "Prompt del agente delegado") y el delegado lo aplica a **sus** corridas
  delegadas. Sin esto se elige el transporte una vez y se pierde en el primer repo.
- **Precedencia: un orden total de cinco niveles**, no dos reglas que se cruzan — (1) override
  conversacional **específico del repo** → (2) override conversacional **global** → (3) **config del
  repo** (`<repo>/.specify/config.yml`) → (4) **config de la orquestación** (`manifest.yml`) →
  (5) **default** `cli`. Fija a la vez el eje de **fuente** (override sobre config) y el de **nivel**
  (repo sobre global), así que ninguna combinación queda sin resolver.
- **El fan-out por repos del propio orquestador queda excluido**: no se convierte a panes, se
  despacha con el mecanismo de subagentes vigente. Multi-repo, presupuesto de layout y escrituras
  paralelas **no están probados** — es una exclusión declarada, no un olvido.

> Predicado de capacidad, durabilidad del override y el detalle del orden total:
> `reference.md` → "Transporte de las corridas delegadas".
...
| "en panes" / "sigue por CLI" (global, o "en `<repo>` por CLI") | override del **transporte** de las corridas delegadas, global o por repo (ver "Transporte de las corridas delegadas") |
...
5. **Agregación.** El estado global solo puede ser verde con **todas** las filas del contrato de integración resueltas y ninguna ausente ni `BLOCKED`. Una fila que falta no es una fila que pasa: si el contrato quedó incompleto, el flujo global se reporta como **no verificado**, no como verde con salvedades.
