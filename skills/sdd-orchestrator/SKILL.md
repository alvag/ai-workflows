...
3. Escribir el **contrato de integración** completo en `<contenedora>/.sdd/<id>/integracion.md` —una fila por cada entrada de `orchestration_tasks`, cubra AC `[integration]` o sea auxiliar— y **congelar su `v1`** acá, con el baseline de cada fila resuelto (`reference.md` → "Contrato de integración"). El `done_when` de cada tarea referencia el ID de su fila en vez de reescribir su criterio, y la referencia solo-lectura del punto 1 de cada repo participante apunta a esa misma fila.
