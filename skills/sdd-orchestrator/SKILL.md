...
3. **Confirmación (checkpoint):** mostrar la lista propuesta y dejar que el usuario agregue/saque repos. **Nunca** se trabaja un repo no confirmado (regla 3).
...
3. **STOP** — si la **revisión cross-model** está activa (ver "Revisión cross-model"), ejecutar `sdd-cross-review` sobre `master-spec.md` (foco en contratos entre servicios y AC `[integration]`) antes de presentar. Presentar la spec madre (con el resumen de crítica, si lo hubo) y pedir aprobación. No avanzar sin ella.
...
4. **STOP** — si la **revisión cross-model** está activa, ejecutar `sdd-cross-review` sobre el `reparto` (artefacto: `manifest.yml`; contexto: `master-spec.md` + los `plan.md` por repo; foco en cobertura AC↔repo, `depends_on` y ciclos del DAG) antes de presentar. Presentar el reparto (tabla repo · branch · AC cubiertos · dependencias, con el resumen de crítica si lo hubo) y pedir aprobación. Al aprobar, poner cada repo en `status: tasks-ready`.
