...
cross_implement: {execution: auto, max_fix_rounds: 2, deadline: 1800}  # política del modo cross (solo si implement_mode: cross); la familia del implementador la fija el conductor (sin `implementer:`); ver paso `implement` y skill cross-implement
```
...
**Objetivo:** descomponer el plan en tareas atómicas, ordenadas, verificables y **autosuficientes** — ejecutables en una sesión fresca sin tener que re-deducir el diseño ni elegir otro enfoque. El modo `subagent` de `implement` **depende** de esta autosuficiencia: cada task debe poder ejecutarla un agente fresco que solo ve **su dossier** —la task, los AC que cubre, las filas `Vn` que cita, el `Produce` de las tasks que consume, los bloques globales que consume y el `## Enfoque` del plan; nada más (`reference.md` → "Reglas de extracción del dossier")—; si no podría, la task está mal escrita. El agente ya no recibe los artefactos completos, así que una task incompleta deja de ejecutarse bien por accidente: se manifiesta como `MISSING_CONTEXT`.
...
- **`subagent`** — cada task **que cambia comportamiento** la implementa un **agente fresco** que recibe el **dossier** de esa task —sus seis piezas ya extraídas, no rutas a artefactos que ir a leer (`reference.md` → "Reglas de extracción del dossier")—, con contexto limpio. El conductor revisa entre tasks y conserva todos los STOPs. Las tasks puramente mecánicas las hace él inline y lo anuncia: es el mismo principio que ya excluye a *trivial* del despacho —delegar un cambio que se termina en un par de tool calls cuesta más que hacerlo—, aplicado por task en vez de por corrida (ver `implement` → paso 1 del modo).
...
2. **Aplicar cambios** según el **modo de ejecución** resuelto (ver arriba). Al iniciar este paso, poner `status: implementing` en el header del `plan.md`. En todos los modos: marcar cada task `- [x]` al completarla (es el detalle fino del progreso que `resume` usa para saber por dónde seguir) y reutilizar lo identificado en `analyze`.
   ...
   - **Modo `subagent`** (loop por task, **siempre secuencial** — un solo working tree; despachar en paralelo garantiza colisiones):
     ...
     1. **No toda task merece un agente.** Una task **puramente mecánica** —config, copy, wiring sin seam testeable: la misma categoría cuyos Pasos el paso `tasks` deja colapsados a 1-2 líneas— la implementa el conductor **inline**. Despachar dos agentes (implementer y reviewer) para un cambio que se termina en un par de tool calls cuesta más de lo que compra, y su garantía vive igual en `verify`. El loop de abajo es para las tasks que **cambian comportamiento**. Decidirlo **por adelantado**, en el pre-flight scan, y **anunciar en una línea** cuáles van inline: un modo `subagent` que ejecuta la mitad de las tasks inline sin decirlo es una sustitución silenciosa del modo elegido.
     2. Por cada task `[ ]` restante, en orden: **armar y validar su dossier** (`reference.md` → "Lado conductor (antes de despachar)"): si `validar-para-despacho` bloquea, la task **no se despacha** — se corrige el artefacto y se vuelve a validar y despachar un **agente fresco** con la plantilla de `reference.md` → "Prompt del subagente por task" (ahí también: cómo despachar según el entorno). El agente implementa **solo esa task**, corre la instrucción de la fila `Vn` que el dossier le interpola —el campo `Verificar` lleva solo el id, no un comando—, y devuelve el reporte estructurado (`STATUS`/`FILES`/`VERIFY`/`NOTES`). No commitea ni toca `.plans/`/`.specify/`.
     3. Al volver: validar `FILES` contra `git status --porcelain` y sumarlos a `code_touched` (regla 8). **Revisar el diff de la task** antes de aceptarlo (disciplina de `receiving-code-review`). Si el entorno puede despachar otro agente fresco, hacerlo con un **reviewer por-task** (plantilla en `reference.md` → "Prompt del subagente reviewer"): recibe el diff + **el mismo dossier que el implementer** —no los artefactos completos— y reporta **spec ✅** (¿cumple los AC que la task habilita?) y **calidad ✅** (¿sin code smells, sigue los patrones del repo?). **Ambos** verdes para marcar la task `- [x]`. Si el reviewer marca ⚠️ "no verificable desde el diff" (un requisito que vive en código no tocado), **no bloquea**: lo resuelve el conductor antes de marcar. Sin capacidad de despachar el reviewer → caer a la **revisión liviana del diff** por el propio conductor, con un aviso de una línea (degradación, regla 6).
     4. Si `STATUS: failed`, o el reviewer reprueba spec/calidad: **máximo 1 reintento**, re-despachando al implementer con el feedback concreto de qué corregir. Si vuelve a fallar, parar y escalar al usuario con el estado — nunca un loop abierto.
     5. Red de seguridad: si el reporte falta o no parsea, clasificar por `git status` + diff; las marcas `[x]` y el `status` del header siguen siendo la fuente de verdad del progreso. **Ante una compactación de contexto**, confiar en esa fuente persistida (`status` + marcas `[x]` + `git log`), no en la memoria de la sesión.
     6. Scratch opcional: si el reporte del implementer/reviewer aporta valor para auditoría o retomado, guardarlo en `.plans/<id>/work/Tn-*.md`. Ese directorio es **scratch local**: no se commitea, no reemplaza `tasks.md`, no contiene `progress.md`, y nunca decide qué task está completa.
