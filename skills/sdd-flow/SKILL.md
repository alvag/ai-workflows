...
1. **Publicar los hallazgos, si hay.** Si `hallazgos.md` no existe o no tiene entradas, este paso no redacta, no pregunta ni publica, y el archivado sigue. Si tiene, redactar un comentario para el TL/PO en el **ticket de desarrollo** `<id>` —no en una subtarea de spec— y presentarlo en un STOP donde el usuario cura la lista y confirma **esa** publicación: la confirmación del archivado no la sustituye. Sin Jira, sin clave, sin escritura o si la escritura falla, se entrega listo para pegar: **nunca bloquea** el archivado. Un archivado delegado por `sdd-orchestrator` no publica, y lo dice. Forma del comentario, línea de cierre, menciones y degradaciones: `reference.md` → "Hallazgos fuera de alcance".
