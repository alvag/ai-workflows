...
- **`subagent`** — cada task **que cambia comportamiento** la implementa un **agente fresco** que solo lee los artefactos (spec/plan/su task), con contexto limpio. El conductor revisa entre tasks y conserva todos los STOPs. Las tasks puramente mecánicas las hace él inline y lo anuncia: es el mismo principio que ya excluye a *trivial* del despacho —delegar un cambio que se termina en un par de tool calls cuesta más que hacerlo—, aplicado por task en vez de por corrida (ver `implement` → paso 1 del modo).
...
2. **Aplicar cambios** según el **modo de ejecución** resuelto (ver arriba). Al iniciar este paso, poner `status: implementing` en el header del `plan.md`. En todos los modos: marcar cada task `- [x]` al completarla (es el detalle fino del progreso que `resume` usa para saber por dónde seguir) y reutilizar lo identificado en `analyze`.
   ...
   - **Modo `subagent`** (loop por task, **siempre secuencial** — un solo working tree; despachar en paralelo garantiza colisiones):
     ...
     2. Por cada task `[ ]` restante, en orden, despachar un **agente fresco** con la plantilla de `reference.md` → "Prompt del subagente por task" (ahí también: cómo despachar según el entorno). El agente implementa **solo esa task**, corre su comando de Verificar, y devuelve el reporte estructurado (`STATUS`/`FILES`/`VERIFY`/`NOTES`). No commitea ni toca `.plans/`/`.specify/`.
