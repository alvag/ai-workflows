...
**Objetivo:** descomponer el plan en tareas atómicas, ordenadas, verificables y **autosuficientes** — ejecutables en una sesión fresca sin tener que re-deducir el diseño ni elegir otro enfoque. Dos consumidores **dependen** de esa autosuficiencia: la **Vía B** de `implement` (bootstrap en sesión fresca, también la que corre el agente delegado de `sdd-orchestrator`) y el modo **`cross`**, donde `cross-implement` congela las tasks como work order y las manda a otra familia. En ambos, quien ejecuta no estuvo en la conversación que produjo el plan: una task que solo se entiende con ese contexto está mal escrita.
