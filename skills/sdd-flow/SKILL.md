...
1b. **Bifurcar por el estado del pedido, antes de toda otra rama.** Con el flujo ya elegido, la **primera** comprobación del paso es el estado del paquete `pedido/`, y de su resultado sale por dónde **enrutar**. Va antes que cualquier rama vigente —incluida la de `antecedentes.md` con `busqueda: terminal`—, porque un flujo terminal puede ocultar un marcador con el paquete ausente, y ahí el fallo cerrado quedaría sin disparar.
   ...
   - **Y después, el registro contra sí mismo y contra la sede de los criterios.** Con la celda «presente y legible», `resume` corre **las mismas comprobaciones deterministas que `specify`**, porque lee los mismos artefactos y es el otro punto donde se decide si el flujo sigue. Un bloque que solo se invoca en la puerta que lo estrenó deja sin mirar todo lo que cambió entre una corrida y la siguiente.
     ...
     <!-- invoca: pedido-referencias -->
     <!-- invoca: pedido-criterio -->
     **Cuál es la sede de los criterios, y cuándo su ausencia es un fallo.** La condición **no** es «si existe `spec.md`»: leerla así confunde un flujo pre-`specify` con uno cuya spec fue borrada, y en el segundo caso **saltea la comprobación justo cuando más hace falta**. La condición se lee del artefacto: **si `## traza` tiene al menos un criterio, la sede tiene que existir** —`spec.md`, o `plan.md` en la rama trivial— y su ausencia es un fallo con salida practicable, **retomar en `specify`** a rehacer la spec contra el registro. Si la traza no tiene ningún criterio, todavía no se adjudicó nada: `pedido-referencias` y `pedido-criterio` no se invocan y se declara por qué.
     **Qué hace `resume` con cada `1`, y no se copia de un bloque a otro.** Solo el de `pedido-digest` es **cuarentena**, porque es el único que evidencia una **reescritura de lo ya confirmado**. El de `pedido-unicidad`, el de `pedido-referencias` y el de `pedido-criterio` **no lo son**: el paquete está intacto y lo que quedó mal es el registro o su correspondencia con la spec, que son reparables —el `literal.jsonl` es lo irreemplazable, el resto se re-deriva—. Su salida es **retomar en el gate de `specify`** con la violación a la vista. Mandar el paquete a cuarentena ahí perdería el trabajo sin arreglar nada. Con `3` de `pedido-criterio` los hashes quedan **sin comprobar** y se informa así, igual que con la cadena.
