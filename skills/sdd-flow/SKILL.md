...
1b. **Bifurcar por el estado del pedido, antes de toda otra rama.** Con el flujo ya elegido, la **primera** comprobación del paso es el estado del paquete `pedido/`, y de su resultado sale por dónde **enrutar**. Va antes que cualquier rama vigente —incluida la de `antecedentes.md` con `busqueda: terminal`—, porque un flujo terminal puede ocultar un marcador con el paquete ausente, y ahí el fallo cerrado quedaría sin disparar.
   ...
   - **Y después, si el flujo ya tiene sede de criterios, el hash de cada criterio contra su texto.** Solo si la celda resultó «presente y legible» **y** existe `spec.md` —o `plan.md` en la rama trivial—: un flujo que todavía no llegó a `specify` no tiene contra qué comparar, y correrlo ahí solo produciría un `3` sin información.
     <!-- invoca: pedido-criterio -->
     **Su `1` no es cuarentena, y el destino es otro.** El paquete está intacto; lo que cambió es el **texto de un criterio** fuera del flujo, así que la adjudicación de `R2` que la traza declara ya no corresponde a lo que la spec dice. La salida practicable es **retomar en el gate de `specify`** para re-adjudicar esos criterios y anexar su evento de corrección — no mover el paquete a un lado, que no arreglaría nada y perdería el trabajo. Con `3` los hashes quedan **sin comprobar** y se informa así, igual que con la cadena. Sin esta comprobación acá, un `resume` sobre una spec editada por fuera devolvía «aplicable» con la traza mintiendo, y ningún paso posterior la volvía a mirar.
