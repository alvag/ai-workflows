...
1. **Escribir/actualizar `handoff.md`** con fase, estado, próximo paso, decisiones y snapshot pre-plan. Preservar siempre ubicación, identidad, contexto, status/etapa/evidencia worktree. Plantilla y momentos: `reference.md` → "Plantilla de `handoff.md`".
...
autoridad del handoff y todo escritor los fusiona, incluso cuando ya existe `plan.md`. Se escriben
en estos momentos: decisión `current` al cerrar el checkpoint; intención `worktree` al cerrarlo; estado
...
vivas**, y quien consume el detector lo corre antes de su primer efecto sobre la plataforma. El
criterio vive acá, escrito y autocontenido: esta skill se instala como copia y corre sobre
repositorios ajenos, así que no puede depender de la ruta de ningún script de otro repositorio.

**Esta es la sede del detector, y sus consumidores la leen**: hoy, la admisión de incidentes, que
conserva una copia derivada de esta sección. **`sdd-flow` no la ejecuta para elegir ni bloquear el
despacho de workers**: sus workers van por línea de comandos, haya o no una plataforma viva. No hay
ninguna dependencia de ejecución sobre un script de otro repositorio, que es lo que permite que una
instalación suelta aplique el criterio sobre un repositorio ajeno.
...
**El detector emite `headless` con su causa, y qué hacer con eso lo decide quien lo consume.** La
admisión de incidentes lo trata como **parada** por cualquiera de sus causas, porque su paso
siguiente necesita un agente interactivo al que despachar. Una causa pesa distinto que las demás: con
**las dos identidades vivas** y sin override no hay observación que diga cuál es el anfitrión, y
elegir una sería adivinar sobre qué máquina se crean recursos. `sdd-flow` no consulta el detector
para despachar sus workers, así que ese empate tampoco lo detiene: sus workers van por línea de
comandos igual.
...
| sin override, **ninguna** identidad `resuelve` | emite `headless` con la causa de cada identidad |

**La matriz es total sobre las dos señales, y por eso tiene seis filas y no cinco.** Las cinco
primeras cubren override y empate; la sexta cubre el caso en que **ninguna** identidad resuelve:
el detector emite `headless`, y qué se hace con eso lo decide su consumidor. Sin esa fila la matriz
dejaba un caso sin destino, y un caso sin destino lo resuelve quien implementa, no quien especifica.
