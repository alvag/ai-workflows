...
description: >-
  ...
  de un gate; la embeben sdd-flow y sdd-orchestrator. Directa: /cross-review con ruta; sin ruta activa
  ...
  excluye diffs, PRs y código. No invocarla espontáneamente: solo por pedido explícito del usuario o
  desde una skill SDD.
---
...
todavía no hay terminal: se devuelve el carrier. Tras la decisión humana terminal, la llamadora
embebida finaliza el único manifest; en directo/draft lo hace esta skill. Todos siguen la matriz de
comparabilidad, creación sin reemplazo y orden de retiro de `reference.md` → "Manifest de corrida".
