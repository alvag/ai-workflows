...
las cinco opciones; el protocolo completo está en `reference.md` → "Tandas y salida de rondas".
...
Una sola de las dieciocho es **autocurva**, y no es ninguna de las cuatro recién nombradas:
`rechazado → rechazado` al conceder una tanda no cambia el valor del estado, pero **sigue siendo una arista ejecutada** y se registra como tal (ver "Ledger").
