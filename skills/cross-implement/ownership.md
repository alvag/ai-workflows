...
| `UNAVAILABLE` | `host_sandbox_wall`; **antes del writer** | `ENVIRONMENT_FAILURE` | No. | No. | No. | No; se resuelve el bloque actual — no hubo writer, así que no hay cese que confirmar ni delta que cosechar. |
| `UNAVAILABLE` | `host_sandbox_wall`; **posterior al despacho** | `ENVIRONMENT_FAILURE` | No. | No. | No. | No; cese incierto: la secuencia se detiene hasta confirmar el cese y cosechar el delta. |
