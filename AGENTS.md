...
- **`sdd-flow`** — SDD de un solo repo, punta a punta: `constitution → gather-context → specify → clarify → create-branch → plan → tasks → implement → verify`. Los tres artefactos se escriben siempre; lo que varía es la **profundidad** —corta, normal o completa—, que el conductor propone por el tamaño del cambio y fija cuántos gates hay: uno, dos o tres. Con Jira en `"on"`, publica la spec y **no bloquea**: el flujo sigue, y si la aprobación externa trae cambios se ajusta lo ya escrito. El diff se revisa siempre antes del commit, con un revisor de la familia opuesta al autor del código.
- **`sdd-orchestrator`** — SDD multi-repo para un objetivo que cruza 2+ repos. Evalúa el riesgo global con el fold conservador de sus filas de assessment, propone la profundidad **por repo** dentro del reparto, materializa el reparto completo antes de su gate atómico y delega cada plan a `sdd-flow`; una divergencia falla cerrado sin bloquear repos independientes. Conserva el baseline y el sello del contrato de integración que `sdd-flow` retiró: acá el usuario no puede mirar todos los repos a la vez.
...
> | `sdd-flow` · revisión final de diff | **sí** — corre siempre, resuelta por la familia opuesta al **autor del código**. Con una sola familia instalada degrada a un revisor fresco same-family, y el gate declara esa degradación |
>
